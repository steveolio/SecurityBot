...
void Intruder::update() {
    ...
    srand (time(NULL)+50); // initialize random seed
    ...
    channel("Intruder_X").send(_current_intruder_x);
    channel("Intruder_Y").send(_current_intruder_y);
}
...
int Intruder::distance_to_intruder_x(){
    return abs(_securitybot.current_x() - _current_intruder_x);
}

int Intruder::distance_to_intruder_y(){
    return abs(_securitybot.current_y() - _current_intruder_y);
}